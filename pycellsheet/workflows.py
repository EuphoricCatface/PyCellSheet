...
try:
    ...
    from pycellsheet.interfaces.pycs import PycsReader, PycsWriter
    from pycellsheet.interfaces.xlsx import XlsxReader
    from pycellsheet.lib.attrdict import AttrDict
    from pycellsheet.lib.hashing import sign, verify
    from pycellsheet.lib.selection import Selection
    from pycellsheet.lib.typechecks import is_svg, check_shape_validity
    from pycellsheet.lib.csv import csv_reader, convert
    from pycellsheet.lib.file_helpers import \
...
class Workflows:
    ...
    def filepath_open(self, filepath: Path):
        ...
        # Reset macro editor
        self.main_window.sheet_script_panel.macro_editor.clear()
    ...
    def edit_paste_as(self):
        ...
        # The mimetypes that are supported by pycellsheet
    ...
    def format_copy_format(self):
        ...
        clipboard = QApplication.clipboard()
        mime_data = QMimeData()
        mime_data.setData("application/x-pycellsheet-cell-attributes", ca_repr)
        clipboard.setMimeData(mime_data)
    ...
    def format_paste_format(self):
        ...
        if "application/x-pycellsheet-cell-attributes" not in mime_data.formats():
            return

        cas_data = mime_data.data("application/x-pycellsheet-cell-attributes")
