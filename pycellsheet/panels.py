...
class SheetScriptPanel(QDialog):
    ...
    def __init__(self, parent, code_array):
        ...
        self._implicit_default_draft = self.code_array.sheet_scripts_draft[0] \
            if self.code_array.sheet_scripts_draft else None
    ...
    def on_apply(self):
        ...
        if err:
            ...
            self.code_array.sheet_scripts_draft[self.current_table] = None
    ...
    def update_(self):
        ...
        if self.code_array.sheet_scripts_draft[self.current_table] is not None:
            self.macro_editor.setPlainText(self.code_array.sheet_scripts_draft[self.current_table])
    ...
    def persist_current_draft(self):
        ...
        if not self.applied_indicator.applied:
            self.code_array.sheet_scripts_draft[self.current_table] = self.macro_editor.toPlainText()

    def has_unapplied_drafts(self) -> bool:
        """Returns whether any sheet has unapplied draft content."""

        self.persist_current_draft()
        for idx, draft in enumerate(self.code_array.sheet_scripts_draft):
            if draft is None:
                continue
    ...
    def discard_all_drafts(self):
        ...
        for idx in range(len(self.code_array.sheet_scripts_draft)):
            self.code_array.sheet_scripts_draft[idx] = None
        self.update_()
