...
section: manual
parent: ../
title: Tools Macro actions
---

# Tools → Macro submenu

Macro-related insert actions are grouped under `Tools → Macro`.

## Tools → Macro → Insert image

The **`Insert bitmap`** option in the `Tools → Macro` submenu lets the user choose an image file and creates cell code that represents an image and chooses the image cell renderer.

## Tools → Macro → Insert chart

The **`Tools → Macro → Insert chart...`** option provides an easy way of generating
...
The preview is updated when pressing the `Apply` button. If an exception occurs or no Figure object can be retrieved, then an error message is displayed.
